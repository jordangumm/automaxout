...
class Maxout():
    ...
    def mixup(self, X, y, rep_ratio=0.1, num_cycles=1):
        """ Duplicate examples with random replacement of features at certain ratio

        Like dropout, but the replacements are sampled from existing examples

        Inspired by https://github.com/unsky/mixup
        """
        X_tmp = np.copy(X)
        y_tmp = np.copy(y)
        for cycle in range(0, num_cycles+1):
            ...
            for i, row in enumerate(X_new):
                for j, col in enumerate(X_new[i]):
                    if randint(0, 100) <= rep_ratio*100:
                        rep_row = randint(0, len(X_new)-1)
                        X_new[i][j] = X_new[rep_row][j]
            X = np.vstack((X, X_new))
            y = np.append(y, y_tmp)
        return X, y
    ...
    def fit(self, train_X, train_y, val_X, val_y, features, batch_size=10,
        ...
        for epoch_num, epoch in enumerate(range(num_epochs)):
            ...
            if val_loss < best_val_loss:
                self.save_network('model_instances/maxout.pkl')
